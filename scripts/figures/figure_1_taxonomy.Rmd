...
B <- ggplot(ST_dist %>% subset(n>0),aes(x = study,stratum = Species_ST_new,alluvium = Species_ST_new,y=prop,fill = Species_ST_new)) +
  geom_alluvium(aes(fill = Species_ST_new),width=0.5) +
  geom_stratum(width=0.5) + theme_bw_me + xlab("Study") + ylab("Percentage") + ST_scale + theme(legend.position = "bottom",
...
AB <-  cowplot::plot_grid(A,B + theme(legend.position='none'),rel_widths =  c(1,0.65),labels = c("A","B"),ncol=2)  
...
ggsave(filename = "./Project_INTEGRATE/Analysis/Resistance_dynamics/INTEGRATE-resistance-ms/figures/figure_1_taxonomy.png",plot=figure,width = 6,height = 3,dpi=900,bg = 'white')
```
