---
title: "Supplemental Figure 5: Resistance histograms"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
create_MIC_histogram <- function(phenotype_cont, phenotype_log, phenotype_cat = NULL, phenotype_name) {
  ...
  x_axis_name <- ifelse(phenotype_name =="Cefiderocol", "Cefiderocol ZOI (mm)",paste0(phenotype_name, " (µg/mL)"))
  ...
  histogram <- ggplot(data, aes(x = phenotype_cont,fill=phenotype_cat)) +
    ...
    ylab("Isolates") +
    xlab(x_axis_name)  + theme_bw_me + resistance_cat_scale+ theme(axis.text = element_text(size=12),axis.title = element_text(size=12))
    
  } else {
  histogram <- ggplot(data, aes(x = phenotype_log,fill=phenotype_cat)) +
    geom_bar()  +
    ylab("Isolates") +
    xlab(x_axis_name) + scale_x_continuous(breaks=as.numeric(names(renaming_vector)), labels=renaming_vector)  + theme_bw_me + resistance_cat_scale + theme(axis.text = element_text(size=12),axis.title = element_text(size=12))
  }
  return(histogram)
}
...
## Figure
```{r,message=F,error=F,warning=F,fig.width=16.5,fig.height=13.2} 
histogram_figs <- cowplot::plot_grid(plotlist=figures,ncol=3,labels=c(LETTERS[1:length(figures) - 1],""))
histogram_figs
```

```{r}
ggsave(plot = histogram_figs, filename = "./figures/s_figure_5_resistance_histograms.png", width = 16.5, height = 13.2, bg = "white",dpi=900)
```