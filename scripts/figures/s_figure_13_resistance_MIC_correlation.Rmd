...
MIC_variables_no_blbli <- subset(MIC_variables,MIC_variables != 'blbli')
MIC_correlation <-  cor(complete_case %>% select(paste0(MIC_variables_no_blbli,"_log_2")) , method="spearman",use='pairwise.complete.obs') %>% round(.,2)
...
```{r,message=F,error=F,warning=F,fig.width=7.25,fig.height=7.25}
corrfig
```
...
```{r}
ggsave(plot = corrfig, filename = "./figures/s_figure_13_resistance_MIC_correlations.png", width = 7.25, height = 7.25, bg = "white",dpi=900)
```
