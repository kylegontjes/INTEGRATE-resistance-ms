...
```{r}
# Figure X table
mytheme <- ttheme_minimal(core = list(fg_params = list(hjust=0, vjust = 0.5 ,x=0.1, fontsize=8.75),padding = unit(c(4.5, 2), "mm")),
                          colhead = list(fg_params = list(hjust=0, vjust = 0.5, x=0.1, fontsize=8.75, fontface="bold"),padding = unit(c(2, 2), "mm")),
...
figure <- cowplot::plot_grid(A,B,ncol=1,rel_heights = c(0.25,1),labels = c("A","B"))
figure
```


```{r}
ggsave(filename = "./figures/s_figure_2_phylogeographic_clustering_statistics.png",plot = figure,dpi=900,bg='white',height=6.8,width=8.5)
```
