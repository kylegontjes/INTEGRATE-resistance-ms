...
df$KPC_plasmid <- factor(df$KPC_plasmid,levels = get_factor_order_plasmid(mobtyper_prop,'KPC_plasmid'))
mobtyper_prop$KPC_plasmid <- factor(mobtyper_prop$KPC_plasmid,levels = get_factor_order_plasmid(mobtyper_prop,'KPC_plasmid'))

A <- ggplot(data=df ,aes(y=KPC_plasmid,fill=KPC_plasmid)) + geom_bar(stat='count')  + theme_bw_me + ylab("") + labs(fill="KPC_plasmid") + facet_wrap(~study) + plasmid_fill  + xlab("Isolates") + theme(legend.position = 'none') +
...
C <- ggplot(data=df,
       aes(y=Species_ST_new, fill=KPC_plasmid)) +
  geom_bar() +
  xlim(NA, 310.1) +
  theme_bw_me +
  xlab("Isolates") + ylab("") +
...
B <-  ggplot(mobtyper_prop  ,aes(x = study,stratum = KPC_plasmid,alluvium = KPC_plasmid,y=prop,fill = KPC_plasmid)) +
  geom_alluvium(aes(fill = KPC_plasmid),width=0.66,alpha=1) +
