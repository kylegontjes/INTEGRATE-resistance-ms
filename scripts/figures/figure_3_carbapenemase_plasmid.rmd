...
A <- ggplot(data=df ,aes(y=forcats::fct_infreq(KPC_plasmid),fill=KPC_plasmid)) + geom_bar(stat='count') + theme(axis.text.x = element_text(angle=90,hjust = 1,vjust=1)) + theme_bw_me + ylab("") + labs(fill="KPC_plasmid") + facet_wrap(~study) + plasmid_fill  + xlab("Count") + theme(legend.position = 'none') +
  geom_text(stat='count',
            aes(label=..count..),
            hjust=-0.05, # push slightly to the right
            size=3.5)  + xlim(NA,85.1) +
  theme(
...
# C. Proportion of plasmids over study periods
```{r}

mobtyper_prop <-  mobtyper_merged  %>% subset(KPC_plasmid ==1) %>%
  count(study, primary_cluster_id) %>%
  complete(study, primary_cluster_id, fill = list(n = 0)) %>%
  group_by(study) %>%
  mutate(prop = n / sum(n) * 100)
   
mobtyper_prop$primary_cluster_id <- factor(mobtyper_prop$primary_cluster_id,levels = mobtyper_prop %>% arrange(n) %>% group_by(primary_cluster_id) %>% summarise (n = sum(n)) %>% arrange(-n) %>% .$primary_cluster_id)
 
C <-  ggplot(mobtyper_prop  ,aes(x = study,stratum = primary_cluster_id,alluvium = primary_cluster_id,y=prop,fill = primary_cluster_id)) +
  geom_alluvium(aes(fill = primary_cluster_id),width=0.5) +
...
mobtyper_prop_ST258 <-  mobtyper_merged  %>% subset(isolate_no %in% ST258_genomes & KPC_plasmid ==1) %>%
  count(study, primary_cluster_id) %>%
  complete(study, primary_cluster_id, fill = list(n = 0)) %>%
  group_by(study) %>%
  mutate(prop = n / sum(n) * 100)
   
mobtyper_prop_ST258$primary_cluster_id <- factor(mobtyper_prop_ST258$primary_cluster_id,levels = levels(mobtyper_prop$primary_cluster_id))
 
D <-  ggplot(mobtyper_prop_ST258  ,aes(x = study,stratum = primary_cluster_id,alluvium = primary_cluster_id,y=prop,fill = primary_cluster_id)) +
  geom_alluvium(aes(fill = primary_cluster_id),width=0.5) +
  geom_stratum(width=0.5) + theme_bw_me + xlab("Study") + ylab("Percentage")  + theme(legend.position = "bottom",
                  axis.text =   element_text(size=12, color="black"),
                  axis.title = element_text(size = 12, color="black"),
                  legend.text =   element_text(size=8, color="black"),
                  legend.title = element_text(size = 10, color="black"),
                  plot.title = element_text(size = 12, color="black")) + scale_y_continuous(breaks = c(0,20,40,60,80,100)) +  plasmid_fill + ggtitle("ST258") 
D
```  

```{r,fig.width=7,fig.height=10.5}  
AB <-  cowplot::plot_grid(A,B,ncol=1,rel_heights = c(1,.667),labels = c("A","B"),label_size=12)  
CD <-  cowplot::plot_grid(C + theme(legend.position = 'none',axis.title.x = element_blank()),D + theme(legend.position = 'none',axis.title.x = element_blank()),ncol=2,labels = c("C","D"),label_size = 12,rel_widths = c(1,1))
figure_3 <-  cowplot::plot_grid(AB,CD,ncol=1,rel_heights = c(1,.33),labels = c("",""),label_size=12)  
figure_3

ggsave(filename = "./Project_INTEGRATE/Analysis/Resistance_dynamics/INTEGRATE-resistance-ms/figures/figure_3_carbapenemase_plasmids.png",plot=figure_3,width = 7,height = 10.5,dpi=900,bg = 'white')
```
