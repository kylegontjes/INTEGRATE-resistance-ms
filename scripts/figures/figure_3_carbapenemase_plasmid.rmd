...
A <- ggplot(data=df ,aes(y=forcats::fct_infreq(KPC_plasmid),fill=KPC_plasmid)) + geom_bar(stat='count')  + theme_bw_me + ylab("") + labs(fill="KPC_plasmid") + facet_wrap(~study) + plasmid_fill  + xlab("Count") + theme(legend.position = 'none') +
...
CD <-  cowplot::plot_grid(C + theme(legend.position = 'none',axis.title.x = element_blank()),D + theme(legend.position = 'none',axis.title.x = element_blank()),NULL,ncol=3,labels = c("C","D",""),label_size = 12,rel_widths = c(1,1,0.5))
