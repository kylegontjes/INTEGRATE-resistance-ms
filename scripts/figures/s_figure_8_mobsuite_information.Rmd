---
title: "Supplemental Figure 8: Plasmid data"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
```{r}
# Packages
packages <- c("tidyverse","gridExtra","kableExtra","tableone","cowplot","ComplexHeatmap",'scales')
...
mobsuite_contigs <-  readRDS("./data/carbapenemase_content/mobsuite_contig_merged_w_carbapenemase_data.RDS") %>% subset(isolate_no %in% df$isolate_no)  
mobsuite_contigs$primary_cluster_id <- factor(mobsuite_contigs$primary_cluster_id,levels =most_frequent_plasmids)

mobtyper_merged$carbapenemase_md <-  sapply(mobtyper_merged$carbapenemase,format_blaKPC_md)   
```

# A. Study distribution
...
A <- ggplot(data=mobtyper_merged %>% subset(primary_cluster_id %in% KPC_plasmids),aes(x=primary_cluster_id,fill=carbapenemase_plasmid)) + geom_bar(stat='count')  + KPC_plasmid_scale  + 
  ...
  labs(x="Primary MOB-cluster",y="Isolates",fill="blaKPC-containing plasmid") + facet_grid(~study) + scale_y_continuous(breaks = breaks_pretty(9))
A
```
...
```{r} 
B <- ggplot(data=mobtyper_merged %>% subset(primary_cluster_id %in% KPC_plasmids) %>% subset(carbapenemase_plasmid==T),aes(x=primary_cluster_id,fill=carbapenemase_md)) + geom_bar(stat='count')  + carbapenemase_scale_2_row + 
  theme_bw_me + theme(axis.text.x = element_text(angle=45,hjust=1),legend.text = element_markdown()) + 
  labs(x="Primary MOB-cluster",y="Isolates",fill="Carbapenemase")   + facet_grid(~study)  + scale_y_continuous(breaks = breaks_pretty(10))
...
C <-  ggplot(data=mobtyper_merged %>% subset(primary_cluster_id %in% KPC_plasmids),aes(x=primary_cluster_id,fill=carbapenemase_plasmid),y=num_contigs) + geom_boxplot(aes(y=num_contigs)) + KPC_plasmid_scale + 
  theme_bw_me + theme(axis.text.x = element_text(angle=45,hjust=1)) + 
  labs(x="",y="Number of contigs",fill="Carbapenemase-containing plasmid")   + scale_y_continuous(breaks = breaks_pretty(6))
...
D <-  ggplot(data=mobtyper_merged %>% subset(primary_cluster_id %in% KPC_plasmids),aes(x=primary_cluster_id,fill=carbapenemase_plasmid),y=size) + geom_boxplot(aes(y=size))  + KPC_plasmid_scale + 
  theme_bw_me + theme(axis.text.x = element_text(angle=45,hjust=1)) + 
  labs(x="",y="Size of plasmid (bp)",fill="blaKPC-containing plasmid")  + scale_y_continuous(breaks = c(0,25000,50000,100000,150000,200000,250000,300000,350000))
...
```{r}
## Predicted mobility 
mobtyper_merged$KPC_plasmid_name <- ifelse(mobtyper_merged$carbapenemase_plasmid ==T,"Carbapenemase plasmid","Non-carbapenemase plasmid")
f <- ggplot(data=mobtyper_merged%>% subset(primary_cluster_id %in% KPC_plasmids),aes(x=primary_cluster_id,fill=predicted_mobility))  + geom_bar()+ facet_wrap(~KPC_plasmid_name)  +   labs(x="",y="Isolates",fill="Predicted mobility") + theme_bw_me + 
  theme_bw_me + theme(axis.text.x = element_text(angle=45,hjust=1),
                      strip.background = element_rect(color = 'black',linewidth = 1)) + mobility_scale + scale_y_continuous(breaks = breaks_pretty(6))
...
G <-    ggplot(data=mobtyper_merged%>% subset(primary_cluster_id %in% KPC_plasmids),aes(x=primary_cluster_id,fill=observed_host_range_ncbi_name))  + geom_bar()+ facet_wrap(~KPC_plasmid_name)    +  labs(x="",y="Isolates",fill="Observed host range")+ theme_bw_me + 
  theme_bw_me + theme(axis.text.x = element_text(angle=45,hjust=1),
                      strip.background = element_rect(color = 'black',linewidth = 1)) + host_range_scale  + scale_y_continuous(breaks = breaks_pretty(6))
...
fig_histograms <- cowplot::plot_grid(A,B ,NULL,ncol=3,labels = c("A","B",""),rel_widths = c(1,1,0.125))
...
fig_facet_histograms <-  cowplot::plot_grid(f,G,NULL,ncol=3,rel_widths=c(1,1,0.125),labels = c("F","G","")) 
...
# Save figure
```{r}
ggsave("./figures/s_figure_8_mobtyper_plasmid_overview.png",plot=figure, units='in',height=18.75,width=15,dpi=900,bg='white')
```
