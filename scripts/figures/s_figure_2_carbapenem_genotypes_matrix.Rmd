---
title: "Supplemental Figure 2: Carbapenem resistance-associated genotype matrix"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
row_annots <- rowAnnotation(Study = df$study,`Major STs` = df$major_STs,
                            ...
                            col = list(Study = c('2014-15'='blue','2021-23'='red'),Carbapenemase = genotype_colors ,`Major STs`=ST_palette %>% subset(names(.) %in% c(unique(df$major_STs)))), annotation_name_gp = gpar(fontsize = 8),annotation_legend_param = list(
...
# Save
```{r}
ggsave(filename = "./figures/s_figure_2_carbapenem_resistance_associated_genotypes_matrix.png",plot=gghtmp,width = 8.5,height = 10.2,dpi=900,bg = 'white')
```
