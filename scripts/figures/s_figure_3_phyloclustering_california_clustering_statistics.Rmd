---
title: "Supplemental Figure 3: Phylogenetic clustering statistics"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
general_statistics$cluster_median_range <- ifelse(general_statistics$clusters ==1,general_statistics$cluster_size_median, paste0(general_statistics$cluster_size_median," (",general_statistics$cluster_size_range,")"))  
...
clustering_data_melt <- clustering_data %>% select(-n_tips,n_participants,-type,-n_participants) %>%  reshape2::melt(id.vars = c("ST","cluster_name"))  %>% subset(variable %in% c("n_PCMP","n_INTEGRATE","n_public"))
...
clustering_data_melt$variable <- factor(clustering_data_melt$variable, levels = rev(c("2014-15 study",'2021-23 study','Public genomes')))
...
B <- ggplot(data=clustering_data_melt ,aes(x=value,fill=variable,y=cluster_name)) + geom_bar(stat='identity') + facet_wrap(~ST,scales = "free_x") + theme_bw_me + xlab("Isolates") + ylab("Phylogenetic clustering of California isolates") +   study_scale_fill
```

```{r,fig.width=8.35,fig.height=6.68}
figure <- cowplot::plot_grid(A,B + theme(legend.position='right'),ncol=1,rel_heights = c(0.275,1),labels = c("A","B"),label_size = 12)
figure
```

```{r}
ggsave(filename = "./figures/s_figure_3_phylogeographic_clustering_statistics.png",plot = figure,dpi=900,bg='white',height=6.88,width=8.35)
```
