...
amrfinder_matrix_beta <- amrfinder_matrix %>% select(any_of(sort(unique(amrfinder_beta_bla$element_symbol))))
amrfinder_matrix_carb <- amrfinder_matrix %>% select(any_of(sort(unique(amrfinder_carb_bla$element_symbol))))
...
df$Species_ST_new <- factor(df$Species_ST_new,levels = get_factor_order(ST_dist,'Species_ST_new'))

C <- ggplot(data=df,
       aes(y=Species_ST_new, fill=carbapenemase)) +
  geom_bar() +
  xlim(NA, 310.1) +
  theme_bw_me +
  xlab("Isolates") + ylab("") +
