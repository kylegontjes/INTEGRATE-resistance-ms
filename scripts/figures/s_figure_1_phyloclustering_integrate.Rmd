...
# Trees 
```{r}
tr_ST14 <- read.tree("./data/phylogeny/phylogeographic/ST14/phylogeographic_ST14_pruned.treefile") %>% drop.tip("INT_CRE_1243") 
tr_ST17 <- read.tree("./data/phylogeny/phylogeographic/ST17/phylogeographic_ST17_pruned.treefile") %>% drop.tip("INT_CRE_1243")  
tr_ST307 <- read.tree("./data/phylogeny/phylogeographic/ST307/phylogeographic_ST307_pruned.treefile") %>% drop.tip("INT_CRE_1243") 
tr_ST258 <- read.tree("./data/phylogeny/phylogeographic/ST258/phylogeographic_ST258_pruned.treefile") %>% drop.tip("INT_CRE_1243")  
``` 
...
study_scale <- scale_fill_manual(breaks = c("2014-15 Study","2021-23 Study","Public California genomes","Other public genomes"),values = c("blue","brown","black","white"),name = "Type of assembly",guide = guide_legend(nrow=4,order=4),drop=FALSE)

study_scale_color <- scale_color_manual(breaks = c("2014-15 Study","2021-23 Study","Public California genomes","Other public genomes"),values = c("blue","brown","black","white"),name = "Type of assembly",guide = guide_legend(nrow=4,order=4),drop=FALSE)

study_scale_color <- scale_color_manual(breaks = c("2014-15 Study","2021-23 Study","Public genomes"),values = c("blue","brown","black"),name = "California assemblies",guide = guide_legend(nrow=3,order=4),drop=FALSE)
```
...
## Function
```{r}
get_phylogeny <- function(st,tr,df,pcdf,col_offset,ymax,point_size,line_width,colname_offset_y,y_tscale,offset_tscale){ 
  tr$edge.length <- scales::pseudo_log_trans(base=10)$transform(tr$edge.length)
  p <- ggtree(tr)  
  tip_x <- max(p$data$x) + (max(p$data$x) * .015)
  tip_x_start <- (max(p$data$x) * .015)
  df_ST <- df %>% subset(ST==st & isolate_no %in% tr$tip.label)
  CA_isolates <- df_ST %>% subset(california==T) %>% .$isolate_no
  tr_data <- as_tibble(tr) %>% left_join(.,df_ST, by = c("label" = "isolate_no")) 
  painted_tree <- phyloAMR::paint_tree_with_states(parent_child_df = pcdf, tr = tr, scale_labels = c("California","Other"),legend_name = "California",scale_colors = c("red","#CCCCCC"))
  ...
  painted_tip_tree <- painted_tip_tree 
  ...
  painted_tip_tree_dashed <- painted_tip_tree + geom_segment(
    data = tip_data,  
    aes(x = x+tip_x_start, y = y, yend = y,color=type),  
    xend = tip_x,        
    linetype = "dashed",
    alpha = 1,
  ...
  p0 <-  gheatmap(painted_tip_tree_dashed , df_ST %>% select(collection_decade) %>% `colnames<-`("Decade"), color=NULL, width=0.05, colnames_position = 'top', colnames_angle = 90, colnames_offset_y = colname_offset_y, hjust=0) + decade_scale + ylim(NA,ymax)  
  p0.1 <- p0+ggnewscale::new_scale_fill()
  figure <- gheatmap(p0.1, df_ST %>% select(continent)  %>% `colnames<-`("Continent") ,color=NULL,width=0.05,offset=col_offset,colnames_position = 'top',colnames_angle = 90,colnames_offset_y = colname_offset_y,hjust=0) + continent_scale + ylim(NA,ymax) + theme(plot.margin = unit(c(0, 0, -0.33, 0), "cm")) + geom_treescale(y = y_tscale, offset = offset_tscale)
  return(figure)
}
```
...
## ST258
```{r,fig.height=7.5} 
A <- get_phylogeny(st='ST258',tr=tr_ST258,df = combined_phylogeographic_df, pcdf = CA_asr$ST258$parent_child_df, col_offset = .00024,ymax = 1840,point_size=0.1,line_width=0.1,colname_offset_y=2,y_tscale=-35,offset_tscale=5)
A  + theme(legend.position='none')  
```

# ST307
```{r,fig.height=7.5}
B <- get_phylogeny(st='ST307',tr=tr_ST307,df = combined_phylogeographic_df, pcdf = CA_asr$ST307$parent_child_df, col_offset = 0.00021, ymax = 1030,point_size=0.1,line_width=0.1,colname_offset_y=2,y_tscale=-25,offset_tscale=4)
B + theme(legend.position='none')
```

## ST14
```{r,fig.height=7.5}
C <- get_phylogeny(st='ST14',tr=tr_ST14,df = combined_phylogeographic_df, pcdf = CA_asr$ST14$parent_child_df, col_offset = 0.00082,ymax = 700,point_size=0.1,line_width=0.1,colname_offset_y=1,y_tscale=-15,offset_tscale=2.5)
C  + theme(legend.position='none')
```

## ST17
```{r,fig.height=7.5} 
D <- get_phylogeny(st='ST17',tr=tr_ST17,df = combined_phylogeographic_df, pcdf = CA_asr$ST17$parent_child_df, col_offset = 0.000305,ymax = 610,point_size=0.1,line_width=0.1,colname_offset_y=1,y_tscale=-15,offset_tscale=2.5)
D + theme(legend.position='none')
```

# Create plots to grab legend! 
```{r}
decade <- ggplot(data=combined_phylogeographic_df,aes(x=collection_decade,fill=collection_decade)) + geom_bar(stat='count') + decade_scale + theme_bw_me + theme(legend.title.position = 'top')
continent <- ggplot(data=combined_phylogeographic_df,aes(x=continent,fill=continent)) + geom_bar(stat='count') + continent_scale  + theme_bw_me+ theme(legend.title.position = 'top')
study <- ggplot(data=combined_phylogeographic_df,aes(x=type_CA,color=type)) + geom_point(stat='count') + study_scale_color + theme_bw_me+ theme(legend.title.position = 'top')
california <- ggplot(data=combined_phylogeographic_df,aes(x=california,fill=california)) + geom_bar(stat='count') + california_scale + theme_bw_me+ theme(legend.title.position = 'top')
...
# Create figure
```{r,fig.width=10.667,fig.height=16}
...
```{r}
ggsave(filename = "./figures/s_figure_1_phylogeographic.png",plot = figure,dpi=900,bg='white',height=16,width=10.667)
```
