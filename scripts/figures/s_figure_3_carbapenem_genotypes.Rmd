...
porin_bla_genotype <- ComplexHeatmap::Heatmap(df_carb_omp %>%  select(-carbapenemase) %>% mutate_all(as.numeric) %>% select_if(colSums(.,na.rm = T)>0) %>% subset(is.na(rowSums(.))==F) %>% `colnames<-`(recode(colnames(.),"ompK35_rare_truncation"='Rare ompK35 truncation',"ompK36_rare_truncation"='Rare ompK36 truncation') %>% gsub("Rare_","Rare ",.)) ,col=c("white","black"),show_row_names = F,show_row_dend = F,show_column_dend = F,name = "Genotype",right_annotation = row_annots,
...
```{r}
ggsave(filename = "./Project_INTEGRATE/Analysis/Resistance_dynamics/INTEGRATE-resistance-ms/figures/s_figure_3_carbapenem_resistance_associated_genotypes.png",plot=gghtmp,width = 8,height = 8,dpi=900,bg = 'white')
```
