...
df_carb_omp <- df %>% select(carbapenemase, any_of(c(unique(amrfinder_beta$element_symbol) ,colnames(kleborate_matrix)))) %>% subset(rownames(.) %in% df_202123$isolate_no)
...
row_annots <- rowAnnotation(`Major STs` = df_202123$major_STs,
                            Carbapenemase = df_202123$carbapenemase,
                            col = list(Carbapenemase = genotype_colors ,`Major STs`=ST_palette %>% subset(names(.) %in% c(unique(df$major_STs)))), annotation_name_gp = gpar(fontsize = 9),annotation_legend_param = list(
...
```{r}
ggsave(filename = "./Project_INTEGRATE/Analysis/Resistance_dynamics/INTEGRATE-resistance-ms/figures/s_figure_3_carbapenem_resistance_associated_genotypes.png",plot=gghtmp,width = 9,height = 3,dpi=900,bg = 'white')
```
