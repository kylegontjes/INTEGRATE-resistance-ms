...
genotype <- df_carb_omp %>% subset(rownames(.) %in% tree_new_final$tip.label) %>% select(geno_vars) %>% drop_na() %>% select_if(colSums(.)>0) %>% ComplexHeatmap::Heatmap()
...
```{r}
ggsave(filename = "./Project_INTEGRATE/Analysis/Resistance_dynamics/INTEGRATE-resistance-ms/figures/s_figure_X_carbapenem_resistance_associated_genotypes_phylogeny.png",plot=A,width = 11,height = 11,dpi=900,bg = 'white')
``` 
