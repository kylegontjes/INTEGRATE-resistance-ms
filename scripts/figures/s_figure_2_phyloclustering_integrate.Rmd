---
title: "Supplemental Figure 2: Phylogeographic analysis of major INTEGRATE STs in CA"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
#Packages
packages <- c("tidyverse",'ape','ggtree','phytools','ggnewscale')
...
get_phylogeny <- function(st,tr,df,pcdf,col_offset,ymax,point_size,line_width,colname_offset_y,y_tscale,offset_tscale){ 
  ...
  painted_tree_w_new_scale <- painted_tree + new_scale_color()  
...
```{r,fig.height=7.5} 
A <- get_phylogeny(st='ST258',tr=tr_ST258,df = combined_phylogeographic_df, pcdf = CA_asr$ST258$parent_child_df, col_offset = .00023,ymax = 1840,point_size=0.1,line_width=0.1,colname_offset_y=2,y_tscale=-35,offset_tscale=5)
...
california <- ggplot(data=combined_phylogeographic_df,aes(x=collection_year,y=continent,color=california)) + geom_line() + california_scale_color + theme_bw_me+ theme(legend.title.position = 'top')
...
```{r}
ggsave(filename = "./figures/s_figure_2_phylogeographic.png",plot = figure,dpi=900,bg='white',height=16,width=10.667)
```
