---
title: "Curate AMRFinder results"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
1. Add isolate number

2. Add contig 

3. Create a new 'symbol' naming scheme for use in honoring the genotype differences
    ...
    if(x[["type"]] == "AMR" & x[["class"]] == "BETA-LACTAM" & x[["subtype"]] != "POINT"){
      if(grepl("-",name) == F| str_split(name,"-",simplify=T) %>% .[,ncol(.)] %>% grepl("[0-9]", .) ==F){
        if(x[["coverage_of_reference"]] < 100 | x[["identity_to_reference"]] < 100){
          name <- str_split(x[["closest_reference_name"]],pattern = " ",simplify = T) %>% .[,ncol(.)]  
          name <- ifelse(name == 'beta-lactamase',x[["element_symbol"]],paste0('bla',name))
        }
      }
    }
    if(grepl("ampC",x[["element_symbol"]])){
      name <- x[["element_symbol"]]
    }
...
write.csv(x = amrfinder,file="./data/amrfinder/amrfinder_curated.csv",row.names = F)
```
...
get_amrfinder_matrix <- function(amrfinder,isolates){
  ...
  pbmcapply::pbmclapply(unique_genes,FUN =function(x,amrfinder){
    ...
    return(row)},mc.cores=7,amrfinder) %>% do.call(bind_cols,.) %>% as.data.frame %>% `colnames<-`(unique_genes) %>% `rownames<-`(isolates)
}
...
write.csv(x = amrfinder_mat,file="./data/amrfinder/amrfinder_matrix.csv",row.names = T)
```