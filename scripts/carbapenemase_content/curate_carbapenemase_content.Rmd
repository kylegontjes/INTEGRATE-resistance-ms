...
## Merge Mobtyper
```{r}
mobtyper_merged <- bind_rows(mobtyper_old %>% select(isolate_no,num_contigs,size,gc,primary_cluster_id,mash_nearest_neighbor,observed_host_range_ncbi_name, predicted_mobility, `rep_type(s)`,`relaxase_type(s)`, 'mpf_type','orit_type(s)'),
                             mobtyper %>% select(isolate_no,num_contigs,size,gc,primary_cluster_id,mash_nearest_neighbor,observed_host_range_ncbi_name, predicted_mobility, `rep_type(s)`,`relaxase_type(s)`, 'mpf_type','orit_type(s)')) %>% mutate(study =ifelse( grepl("INT",isolate_no),"2021-23",'2014-15'))
...
df$blactamase <- sapply(df$isolate_no,FUN=function(x){
  amrfinder_beta_bla %>% subset(isolate_no == x) %>% .$element_symbol_renamed %>% unique %>% paste0(collapse=" & ")
}) %>% {ifelse(.=="","No blactamase",.)}

df$blactamase_non_carbapenemase <- sapply(df$isolate_no,FUN=function(x){
  amrfinder_beta_bla_non_carb %>% subset(isolate_no == x) %>% .$element_symbol_renamed %>% unique %>% paste0(collapse=" & ")
}) %>% {ifelse(.=="","No blactamase",.)}

df$carbapenemase <- sapply(df$isolate_no,FUN=function(x){
  amrfinder_carb_bla %>% subset(isolate_no == x) %>% .$element_symbol_renamed %>% unique %>%  paste0(collapse=" & ")
}) %>% {ifelse(.=="","No carbapenemase",.)}

df$carbapenemase_family <- sapply(df$isolate_no,FUN=function(x){
    carbapenemase <- amrfinder_carb_bla %>% subset(isolate_no == x) %>% .$element_symbol_renamed
...
carbapenemase_data <- df %>% select(isolate_no,blactamase,blactamase_non_carbapenemase,carbapenemase,carbapenemase_family,carbapenemase_plasmid)
