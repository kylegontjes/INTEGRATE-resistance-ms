---
title: "Results section 1-3: Propsective surveillance of CRKP from California LTACHs and differences across studies"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
# Surveillance in LTACHs

## 2014-15 (period 1) study

1. From August 1 to 25 July 2025
...
1. Per Leigh - Study was from June 7, 2021 to December 31, 2023.
...
# Date
INTEGRATE_full_dataest <- readRDS("./data/dataset/INTEGRATE_metadata_qcd_merged.RDS")
paste0("Minimum date: ",min(INTEGRATE_full_dataest$collect_date,na.rm=T))
paste0("Maximum date: ",max(INTEGRATE_full_dataest$collect_date,na.rm=T))
...
```{r}
df_202123_no_metadata <- df %>% subset(has_full_metadata==F & study =='2021-23')
df_202123_w_metadata <- df %>% subset(has_full_metadata==T & study =='2021-23') 
...
convert_tableone_into_df(dataset = df_w_metadata, vars = c("participant_age_at_first_enc","participant_sex","participant_race","participant_ethnicity","non_hispanic_white"), factorVars = c("participant_sex","participant_race","participant_ethnicity","non_hispanic_white"),strata='study',outcome_names = c("2014-15","2021-23"),nonnormal_vars = 'participant_age_at_first_enc') %>% favorite_kable
...
hospital_study_table_no_par %>% chisq.test(simulate.p.value = T)
```
...
convert_tableone_into_df(dataset=df ,vars = c("source_simplified","blood","other","resp","urine","wound"),strata='study',outcome_names = c('2014-15',"2021-23"),factorVars = c("source_simplified","blood","other","resp","urine","wound"),exact = c("source_simplified","other")) %>% favorite_kable()

paste0("Isolates with other source declaration: ")
df %>% subset(source_simplified == "other") %>% .$source_site
```
