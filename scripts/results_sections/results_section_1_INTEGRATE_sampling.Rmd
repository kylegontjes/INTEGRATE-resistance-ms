...
1. Preferentially select first isolate with metadata
```{r}
df_w_los_metadata <- df %>% subset(has_full_metadata==T) %>% group_by(participant_id) %>% arrange(date) %>% slice(1)

paste0("Number of participants with LOS data: ",nrow(df_w_los_metadata))
table(df_w_los_metadata$study)
 
paste0("Wilcox test for length of stay")
median_IQR("los_before_culture",df_w_los_metadata)
```
