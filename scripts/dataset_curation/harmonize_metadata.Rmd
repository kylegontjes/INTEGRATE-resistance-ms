---
title: "Haromonize metadata and merge datasets"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
output: html_document
---

# Update: Removed absolute paths
...
```{r}
# 2014-15 study
df_201415 <- readRDS("./data/dataset/Penn_KPC_metadata_qcd_merged.RDS") %>% as.data.frame() %>% subset(qc_pass==T  & CRE==T)

# 2021-23 study
df_202123 <-  readRDS("./data/dataset/INTEGRATE_metadata_qcd_merged.RDS") %>% as.data.frame()%>% subset(qc_pass==T & CRE==T)
```
...
df_202123$participant_sex <- recode(df_202123$patient_sex, 'F'='female', 'M'='male')
...
# Lab vantage ID
## Not found in 2014-15 study

```{r}
df_201415$labvantage_id <-  NA
```
...
## Add numeric, log2, categorical, and dichotomous variables
...
# Metadata harmonizing before merging
...
# Recode LTACH by sorting the names and giving them a new number
...
# Resistance using SENSITIRE
df_metadata_all$MERO_IMI_R_SENSITITRE <- ifelse(is.na(df_metadata_all$MERO)==T & is.na(df_metadata_all$IMI) ==T,NA,
                                                ifelse(df_metadata_all$MERO_cat == "Resistant" | df_metadata_all$IMI_cat == "Resistant",1,0))
...
```{r}
saveRDS(df_metadata_all,"./data/dataset/CA_LTACH_CRE_genomes_merged_studies_metadata.RDS")
write_csv(df_metadata_all,"./data/dataset/CA_LTACH_CRE_genomes_merged_studies_metadata.csv")
```
