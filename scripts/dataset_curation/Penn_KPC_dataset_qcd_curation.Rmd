---
title: "2014-15 CA CRKP LTACH isolate collection dataset curation"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
# Functions
setwd("~/Desktop/gl_nfs/Project_INTEGRATE/Analysis/Resistance_dynamics/INTEGRATE-resistance-ms")
source("./lib/consistent_themes.R") 
source("./lib/metadata_curation_functions.R")
...
# Load data
1. This is the 459 data provided by UPENN

2. Isolate_no is the specimen-level identifier while participant_id is the patient identifier

3. We subset to the California isolates

```{r} 
df_2014_15 <- readxl::read_excel("./Project_Penn_KPC/Analysis/data/459_patients_data_for_posting.xls")  %>% as.data.frame
...
# We recoded participant 13's initial entry due to incorrect admit and discharge dates

1. Participant 13 had >1 isolate. 

2. Although the age was accurate (i.e., age different correlated with collection date difference), the two isolates had the same admit date, which suggests a possible issue with admit and discharge data

```{r}
# Isolate with weird discharge data
wrong_Cx_data <- df_2014_15 %>% subset(LOSbeforeCx <0) %>% select(isolate_no,Patient_ID,Cx_date,admit_date,discharge_date,LOSbeforeCx,LOS_after_cx)
paste0("Incorrect isolate with bad admit and discharge data: ")
wrong_Cx_data %>% favorite_kable()
...
df_2014_15%>% subset(participant_id %in% wrong_Cx_data$Patient_ID) %>% select(isolate_no,Patient_ID,Cx_date,admit_date,discharge_date,LOSbeforeCx,LOS_after_cx) %>% favorite_kable()

# Create a flag for full metadata (e.g., not those isolates!)
df_2014_15$has_full_metadata <- !df_2014_15$isolate_no %in% wrong_Cx_data$isolate_no
```
...
```{r}
SENSITITRE <- readxl::read_excel("./Project_Penn_KPC/Analysis/Antibiotic_resistance/data/SENSITITRE MICs.xlsx", range = "B1:N460") %>% as.data.frame
...
1. Renamed the variable names

2. Fixed coding issue with inhibitor of MVB

3. Numerize, factored, log transformed MIC data 
...
paste0("MVB has some issues that needed recoding: ")
...
for(i in MIC_variables){
  SENSITITRE[,paste0(i)] <- as.character(SENSITITRE[,paste0(i)] )
}
...
3. See our prior work in ICHE: Zhang H et al., Infection Control & Hospital Epidemiology, 2022 (PMC9883590)

4. Note: We recoded colistin intermediate category as 'susceptible' for sake of fitting the susceptible vs. resistant paradigm
...
SENSITITRE <- SENSITITRE %>% mutate(
  ...
  # Fosfomycin (FOS): Susceptible <=64. We only had >64 reported, which would be classified as at least within the intermediate category
...
MIC_variables_no_CST<- MIC_variables %>% subset(.!= "CST")
for(i in MIC_variables_no_CST){
  SENSITITRE[,paste0(i,"_dich")] <- binarize_MIC(SENSITITRE,paste0(i,"_cat"))
}

## Colistin reclassified intermediate category as susceptible.
...
## Numerical Dichotomous Variable for Resistance Analysis
for(i in MIC_variables_dich){
...
1. This will focus on KPC-inhibiting BL/BLI antibiotics: MVB, CZA, and IR
...
1. Using an Enterobacterales species call list (although it doesn't matter as we're working with Kpn here)

```{r}
# ESCrE list containing Enterobacterales designation
...
# Convert column names to snakecase for easy use
...
# Prioritize Sequence type (ST) data via Kleborate

1. We are prioritizing the dataset from Kleborate not the default from QCD (mlst)

```{r}
kleborate_kpsc_penn <- read_delim("./Project_INTEGRATE/Analysis/Resistance_dynamics/INTEGRATE-resistance-ms/data/kleborate/kpsc_Penn_KPC/klebsiella_pneumo_complex_output.txt")  %>% mutate(isolate_no = gsub("_contigs_l1000","",strain))

ST_data_201415 <- lapply(df_2014_15$isolate_no,FUN=function(x){
  kleb_sub <- kleborate_kpsc_penn %>% subset(isolate_no ==x)
  kleb_sub %>% select(isolate_no,ST)
}) %>% do.call(rbind,.)

df_2014_15 <- left_join(df_2014_15,ST_data_201415) %>% rename(mlst_st=st)
```
...
paste0("Number of qc_passing CRE isolates & full metadata: ",sum(df_2014_15$qc_pass==TRUE & df_2014_15$CRE==TRUE & df_2014_15$has_full_metadata==T))
```

# Save the 2014-15 dataset
...
write_csv(df_2014_15,"./Project_INTEGRATE/Analysis/Resistance_dynamics/INTEGRATE-resistance-ms/data/dataset/Penn_KPC_metadata_qcd_merged.csv")
```
