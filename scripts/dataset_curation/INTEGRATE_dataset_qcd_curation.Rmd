---
title: "2021-23 CA CRKP LTACH isolate collection dataset curation"
author: "Kyle Gontjes"
date: "`r Sys.Date()`"
...
c. Mismatched and other to-be-dropped isoaltes were flagged for removal 

d. Dropped one INT_1259 entry as it was a duplicate!

2. 06/05/26: Removed gontjesr functions

3. 06/22/26: Convert categories to informative naming

4. Concert imipenem non-susceptibilty to resistant category in binary variable
...
1. Used QCD pipeline for processing 
...
1. We retained Enterobacterales species per LLC's suggestions
...
1. A few genomes had possible mismatches. They were dropped.

```{r}
mismatches <- read_xlsx("../../data/INTEGRATE_lab_WGS_species_mismatches_021025_llc.xlsx")  %>% .[-nrow(.),] 
...
2. Duplicated metadata entries were flagged and dropped accordingly

```{r}
duplicated_metadata_entries <- read_csv("../../data/INTEGRATE_duplicated_metadata_entries_120125.csv")
duplicated_metadata_entries %>% favorite_kable()
...
```{r}
# Load
df <- read_delim("../../data/integrate_metadata_20260220.txt")
df <- df %>%  rename(lab_species = species,labvantage_id = labvantage_barcode_bacterial_cell_isolate)
...
# Remove  isolates
## Remove the not delivered
df$not_delivered <- df$labvantage_id=='not delivered'
paste0(subset(df,not_delivered ==T) %>% .$sample_id)
df <- df %>% subset(not_delivered == F) 
...
# Merge sequencing data (QCD) with metadata
...
2. Recode numerical values

3. Numerize and log2 transform

4. Create dichotomous variable

5. Create KPC-inhibiting BL/BLI antibiotics

```{r}  
# Convert names
## Raw names
df_merged <-  dplyr::rename(df_merged,
                            MERO='mero',
                            IMI='imi',
                            CZA='cza',
                            MVB = "mev",
...
MIC_variables_wo_IMI <- MIC_variables %>% subset(.!="IMI")

## Recode numeric values if found in category if numeric is NA
for(i in MIC_variables){
  df_merged[,paste0(i)] <- as.character(ifelse(df_merged[[paste0(i)]] %in% c("S","I","R"),NA, df_merged[[paste0(i)]]))
}
...
for(i in MIC_variables_cat){
  df_merged[,i] <- factor(as.vector(df_merged[[i]]), 
                           levels = c("S","I","R"),
                           labels = c("Susceptible","Intermediate","Resistant"))
}

# Dichotomous variables
## Every resistant variable other than imipenem
for(i in MIC_variables_wo_IMI){
  df_merged[,paste0(i,"_dich_num")] <- sapply(df_merged[[paste0(i,"_cat")]],FUN=function(x){
      ifelse(x %in% c("ERROR",NA),NA,ifelse(x %in% c("Intermediate","Resistant"),1,0))
  })
}

## Imipenem: Dichotomous resistance is the resistant category only, due to lack of measurement
df_merged$IMI_dich_num <- ifelse(df_merged$IMI_cat=="Resistant",1,0)
```
...
```{r}  
df_merged$blbli_dich_num <- ifelse(is.na(df_merged$MVB_dich_num) & is.na(df_merged$IR_dich_num) & is.na(df_merged$CZA_dich_num), NA,
                                   ifelse(df_merged$MVB_dich_num==1| df_merged$IR_dich_num==1 | df_merged$CZA_dich_num == 1, 1, 0))


df_merged$blbli_dich <- ifelse(df_merged$blbli_dich_num==1,"Non-susceptible",
                               ifelse(df_merged$blbli_dich_num==0,"Susceptible",df_merged$blbli_dich_num))
```
  
## Prioritize kleborate's ST data 

1. We are prioritizing kleborate's ST calls over mlst
